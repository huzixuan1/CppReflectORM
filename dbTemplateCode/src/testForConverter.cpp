...
namespace cppToSql {
    ...
    struct TypeConverter
    {
        static std::string converter() {
            static_assert(sizeof(T) == 0, "TypeConverter not implemented for this type!");
            return "UNKNOWN";
            // return "";
        }
    };
...
// TableBuilder, MySQLTableBuilder, TableDirector definitions
class TableBuilder {
public:
...
std::vector<std::pair<std::string, std::string>> getStructFieldsInfo(const T& struct_t) {
    ...
    boost::pfr::for_each_field(
        struct_t, [&fieldsInfo, &names](const auto& field, std::size_t idx) {
            ...
            std::string sqlType = cppToSql::TypeConverter<FieldType>::converter();
            fieldsInfo.emplace_back(fieldName, sqlType);
        });
...
int main() {
    std::cout << "Getting struct fields" << std::endl; 
